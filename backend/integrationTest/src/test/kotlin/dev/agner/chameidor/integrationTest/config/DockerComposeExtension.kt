...
import dev.agner.chameidor.persistence.migration.migrate
            ...
            launch {
                DockerCompose.getContainerByServiceName("mysql-1")
                    ...
                    .execInContainer(
                        ...
                        "-c",
                        """
                            mysql -uroot -Nse 'show tables' chameidor | while read table; do
                               if [ "${'$'}table" != 'flyway_schema_history' ]; then
                                   mysql -e "set FOREIGN_KEY_CHECKS=0; truncate table ${'$'}table" chameidor;
                               fi
                            done
                        """.trimIndent(),
                    )
